...
Visit https://xlwt.readthedocs.io/en/latest/licenses.html for more information of XLWT Package license
"""

# Operate system interface
import os
# need to install(command in cmd): pip install xlwt
# it is an open source package (https://groups.google.com/g/python-excel/c/P6TjJgFVjMI/m/g8d0eWxTBQAJ)
...
def ClassifyFiles(dataFolderPath):
    """
    This function is to classify file names of files from the target folder (where store the txt files)(path) into a
    dictionary
...
def createDataFile(dataFolderPath, fileDict, saveAddress, savingName):
    """
        This function is to generate a exccel file that contains the data in the txt files
    ...
    for participantIndex in range(len(fileDictKeyList)):
        ...
        for trialIndex in range(len(trailList)):
            ...
            with open(os.path.join(dataFolderPath, trailFile))as dataTxt:
                ...
                # set savingAddress
                savingAddress = rf"{saveAddress}\{savingName}.xls"

                # save the excel file at the location
                dylanData.save(savingAddress)
...
if __name__ == '__main__':
    ...
    # give the saving address
    saveAddress = r"C:\Users\Dylan Ding\Desktop\SortedData"

    savingName = "Dylan_Data"

    createDataFile(dataFolderPath, classifiedDict, saveAddress, savingName)
